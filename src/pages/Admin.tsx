            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3 overflow-x-auto pb-2">
                {productCategories.map(cat => (
                  <div key={cat.id} className="group w-52 shrink-0 rounded-xl border border-slate-200 bg-white p-4 transition-all hover:border-navy">
      ...
      {tab === 'documents' && (
        <div className="space-y-4">
          <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
            ...
            <div className="flex flex-wrap gap-3"><button onClick={() => { setSelectedDocumentCategory(null); setIsDocumentCategoryModalOpen(true); }} className="btn-outline flex items-center justify-center gap-2 px-6 py-3 text-sm"><IconRenderer name="plus" className="h-4 w-4" />Add Category</button><button onClick={() => { setSelectedDocument(null); setIsDocumentModalOpen(true); }} className="btn-primary flex items-center justify-center gap-2 px-8 py-3 text-sm"><IconRenderer name="plus" className="h-4 w-4" />Add PDF</button></div>
          </div>
          {documentCategories.length > 0 ? <div className="flex gap-3 overflow-x-auto pb-2"><>{documentCategories.map(category => <div key={category.id} className="group w-52 shrink-0 rounded-xl border border-slate-200 bg-white p-4 hover:border-navy"><div className="flex items-start justify-between"><IconRenderer name="file-text" className="h-6 w-6 text-navy" /><span className="flex gap-1 opacity-0 transition-all group-hover:opacity-100"><button onClick={() => { setSelectedDocumentCategory(category); setIsDocumentCategoryModalOpen(true); }} className="p-1 text-blue-600"><IconRenderer name="user-pen" className="h-3 w-3" /></button><button onClick={() => handleDeleteDocumentCategory(category.id)} className="p-1 text-red-600"><IconRenderer name="trash" className="h-3 w-3" /></button></span></div><p className="mt-2 line-clamp-2 text-xs font-bold text-navy">{category.name}</p></div>)}</></div> : <div className="rounded-xl border border-dashed border-slate-300 bg-white p-6 text-center text-sm font-semibold text-slate-500">No categories yet</div>}
